...
> See below for resume-ready bullets.

**Project:** Dual-Arm Cooperative Manipulation with Deep Reinforcement Learning

Designed and implemented a dual-arm robotic manipulation environment extending the
open-source panda-gym framework. Two Franka Panda robots learn a cooperative cube handover
task — pick, transport, and place — controlled by a single SAC policy trained with
Stable-Baselines3.

**Key contributions:**
- Extended a single-arm PyBullet simulation to support two physically independent robot
  bodies sharing one scene, without duplicating URDF assets.
- Designed a phased dense reward with stage-dependent end-effector shaping to provide
  learning signal across a long-horizon cooperative task.
- Built a minimal Gymnasium-compatible environment stack (robot wrapper, task, env)
  following existing panda-gym patterns.
- Set up an end-to-end RL pipeline with checkpointing, TensorBoard logging, and a
  structured evaluation script reporting success rate and stage-completion statistics.

**Resume bullets (pick and adapt):**

- Extended panda-gym (PyBullet) to a dual-arm cooperative manipulation environment;
  designed phased dense reward enabling SAC policy to learn pick-transport-handover
  across an 8-DoF continuous action space.

- Built Gymnasium-compatible dual-arm RL environment in Python; integrated
  Stable-Baselines3 SAC with checkpoint callbacks and TensorBoard logging for a
  1 M-step training pipeline.

- Implemented cooperative robot task (cube handover) as a single-agent RL problem;
  env achieves X% success rate after Y steps (fill in after training).
